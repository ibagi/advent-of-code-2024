import { Grid, type Coordinates } from "../common/grid";
...
function doSearch(grid: Grid, coord: Coordinates, dir: Direction, searchWord: string) {
    const [x, y] = coord;
    const [dX, dY] = dir;
...
async function partOne() {
    ...
    return grid.toArray()
        .reduce((result, pos) => result + directions
...
async function partTwo() {
    ...
    return grid.toArray()
        ...
        .reduce((result, [x, y]) => {
            const { topLeft, topRight, bottomLeft, bottomRight } = grid.neighbours(x, y);
            const horizontalPattern = [topLeft, topRight, bottomLeft, bottomRight].join('');
            const verticalPattern = [topLeft, bottomLeft, topRight, bottomRight].join('');
            const patternFound = crossPatterns.includes(horizontalPattern) || crossPatterns.includes(verticalPattern);
