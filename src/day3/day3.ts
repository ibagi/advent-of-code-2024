...
function findMaches(input: string, filter?: (m: RegExpExecArray) => boolean) {
  ...
  return matches.filter((m) => !!filter ? filter(m) : true).reduce(
    (sum, match) =>
      match.groups
...
const lookBehind = (text: string) => (match: RegExpExecArray) => {
  ...
  const allow = slice.matchAll(/do\(\)/g).reduce(
    (curr, m) => Math.max(m.index, curr),
    -1,
  );
  const disallow = slice.matchAll(/don\'t\(\)/g).reduce(
    (curr, m) => Math.max(m.index, curr),
    -1,
  );
